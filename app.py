...
if not st.session_state.autenticado:
    ...
    # Contenedor exterior para centrar la tarjeta de login
    outer_c1, outer_c2, outer_c3 = st.columns([0.1, 3.8, 0.1])
    with outer_c2:
        # Abrimos el contenedor general con estilo de tarjeta unificada
        st.markdown(
            """
            <div style="
                background: #12151c;
        ...
        # Usamos columnas internas de Streamlit para separar perfectamente la izquierda y la derecha
        col_banner, col_form = st.columns([1.1, 1], gap="small")
        ...
        with col_form:
            # Contenedor con padding para el formulario interior
            st.markdown(
                """
                <div style="padding: 40px 35px; background: #12151c; display: flex; flex-direction: column; justify-content: center;">
                """,
                unsafe_allow_html=True,
            )
            ...
            st.markdown("</div>", unsafe_allow_html=True)  # Cierre del div derecho
