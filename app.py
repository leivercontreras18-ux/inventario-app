...
FAVICON_URL = "https://qhihqujbzjcuxshndsez.supabase.co/storage/v1/object/public/productos-fotos/1787708461873-removebg-preview%20(1).png"

try:
    _favicon_resp = requests.get(FAVICON_URL, timeout=3)
...
# ==========================================
# CONFIGURACIÓN DE CONTACTO (editar aquí)
# ==========================================
WHATSAPP_BOUTIQUE = st.secrets.get("WHATSAPP_BOUTIQUE", "")
INSTAGRAM_BOUTIQUE = "@lewinboutique"  # ← Cambiar cuando quieras
SLOGAN_BOUTIQUE = "Moda que enamora"    # ← Cambiar cuando quieras
URL_CATALOGO_WEB = "https://lewinboutique.netlify.app"
...
LOGO_URL = "https://qhihqujbzjcuxshndsez.supabase.co/storage/v1/object/public/productos-fotos/1787708461873-removebg-preview.png"
...
def generar_etiquetas_pdf(lista_prendas, tasa_cambio):
    """Genera un PDF con etiquetas listas para imprimir (3 columnas x 8 filas por página A4)."""
    if not PDF_DISPONIBLE:
        return None
    try:
        ...
        for idx, prenda in enumerate(lista_prendas):
            pos = idx % (cols * filas_por_pagina)
            if pos == 0 and idx > 0:
                pdf.add_page()
            ...
            x = margen_x + col * (ancho_etiqueta + espacio_x)
            y = margen_y + fila * (alto_etiqueta + espacio_y)
            ...
            pdf.set_text_color(30, 30, 30)
            nombre = str(prenda["Producto"])[:30]
            pdf.cell(ancho_etiqueta, 4, nombre, align="C")

            pdf.set_font("Helvetica", "", 7)
            pdf.set_text_color(80, 80, 80)
            pdf.set_xy(x + 3, y + 12)
            pdf.cell(20, 3.5, f"ID: {prenda['ID']}")
            pdf.set_xy(x + 3, y + 15)
            pdf.cell(20, 3.5, f"Talla: {prenda['talla']}")
            pdf.set_xy(x + 3, y + 18)
            pdf.cell(30, 3.5, f"Color: {prenda['color']}")
            ...
            if tasa_cambio > 0:
                ...
                pdf.cell(ancho_etiqueta, 3, f"{precio_bs:,.2f} Bs", align="C")
...
.win-kpi-card { background: #ffffff; padding: 20px; border-radius: 16px; box-shadow: 0 4px 20px rgba(74,111,165,0.06); border: 1px solid #eef2f9; position: relative; }
...
.win-btn-row button { background: #f1f5f9 !important; border: 1px solid #e2e8f0 !important; border-radius: 10px !important; color: #4a6fa5 !important; font-size: 16px !important; font-weight: 700 !important; min-height: 38px !important; -webkit-text-fill-color: #4a6fa5 !important; }
.win-btn-row button:hover { background: #4a6fa5 !important; color: #ffffff !important; -webkit-text-fill-color: #ffffff !important; border-color: #4a6fa5 !important; }
...
/* ===== INPUTS Y SELECTORES EN BLANCO ===== */
        ...
        /* SVG (flecha del dropdown) */
        div[data-baseweb="select"] svg,
        div[role="button"][aria-haspopup="listbox"] svg {{
...
if not st.session_state.autenticado and st.session_state.etapa == "bienvenida":
    ...
    hero_html = """
        ...
        .hero-new {
          ...
          background: linear-gradient(135deg, #d0e0f0 0%, #e2eef8 50%, #cadcee 100%);
          border-radius: 32px;
          overflow: hidden;
        ...
        <div class="hero-new">
          ...
          <div class="hero-new-content">
            ...
            <div class="hero-new-desc" style="color: #0f1a2b !important; opacity: 1 !important; font-weight: 700 !important; font-size: 16px !important; font-family: 'Poppins', sans-serif !important; line-height: 1.6 !important; margin-bottom: 35px !important; max-width: 650px !important;">Gestion completa de inventario, ventas, reportes y catalogo visual en una sola plataforma.</div>
...
# =====================================================================================
    ...
    if compacto:
        ...
        st.sidebar.markdown(
            f'<div style="text-align: center; padding: 20px 0 15px 0;"><img src="{LOGO_URL}" style="width: 400px; height: auto;" /></div>',
            unsafe_allow_html=True,
        )
...
# -----------------------------------------------------------------------------
# INICIO (dashboard resumen) - NUEVO DISEÑO
# -----------------------------------------------------------------------------
if menu == "inicio":
    ...
    if not movs_inicio.empty:
        ventas_df = movs_inicio[movs_inicio["tipo"] == "venta"].copy()
        if not ventas_df.empty:
            ventas_df["fecha_dt"] = pd.to_datetime(ventas_df["fecha"], errors="coerce").dt.date
            ventas_hoy = ventas_df[ventas_df["fecha_dt"] == hoy]
            ventas_hoy_monto = float((ventas_hoy["cantidad"] * ventas_hoy["precio_unitario"]).sum())
            ganancia_hoy = float((ventas_hoy["cantidad"] * (ventas_hoy["precio_unitario"] - ventas_hoy["costo_unitario"])).sum())
    ...
    with col_titulo:
        st.markdown(
            f"""<div class="win-greeting">¡Hola, {st.session_state.usuario_actual.capitalize()}! 👋</div>
<div class="win-subtitle">Aquí está el resumen de tu boutique hoy.</div>""",
            unsafe_allow_html=True,
        )
    with col_btn:
        st.markdown("<div style='height:12px;'></div>", unsafe_allow_html=True)
        if st.button("➕ Nueva Venta", use_container_width=True, type="primary", key="btn_nueva_venta_dash"):
...
<div class="win-kpi-label">Ventas de Hoy</div>
<div class="win-kpi-icon" style="background:#e0ecff; color:#4a6fa5;">💰</div>
<div class="win-kpi-value">{moneda(ventas_hoy_monto)}</div>
<div class="win-kpi-change win-up">↑ Hoy</div>
</div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="win-kpi-card">
<div class="win-kpi-label">Ganancia Neta</div>
<div class="win-kpi-icon" style="background:#dcfce7; color:#16a34a;">📈</div>
<div class="win-kpi-value">{moneda(ganancia_hoy)}</div>
<div class="win-kpi-change win-up">↑ Hoy</div>
</div>""", unsafe_allow_html=True)
    with col3:
        st.markdown(f"""<div class="win-kpi-card">
...
<div class="win-kpi-value">{moneda(0)}</div>
<div class="win-kpi-change win-neutral">Próximamente</div>
</div>""", unsafe_allow_html=True)
    with col4:
        st.markdown(f"""<div class="win-kpi-card">
<div class="win-kpi-label">Stock Bajo</div>
<div class="win-kpi-icon" style="background:#fee2e2; color:#dc2626;">⚠️</div>
<div class="win-kpi-value">{stock_bajo_count}</div>
<div class="win-kpi-change win-down">Requiere atención</div>
</div>""", unsafe_allow_html=True)

    st.markdown("<div style='height:10px;'></div>", unsafe_allow_html=True)

    with st.container():
        st.markdown('<div class="win-chart-card">', unsafe_allow_html=True)
        st.markdown('<div class="win-chart-title">📊 Ventas de la semana</div>', unsafe_allow_html=True)

        if not movs_inicio.empty:
            ventas_graf = movs_inicio[movs_inicio["tipo"] == "venta"].copy()
            if not ventas_graf.empty:
                ventas_graf["fecha_dt"] = pd.to_datetime(ventas_graf["fecha"], errors="coerce")
                ventas_graf["dia"] = ventas_graf["fecha_dt"].dt.strftime("%d/%m")
                ventas_graf["monto"] = ventas_graf["cantidad"] * ventas_graf["precio_unitario"]
                serie_semana = ventas_graf.groupby("dia")["monto"].sum().tail(7)
                
                fig_sem = go.Figure()
                fig_sem.add_trace(go.Scatter(
                    x=list(serie_semana.index), y=list(serie_semana.values),
                    mode="lines", line=dict(color="#4a6fa5", width=3, shape="spline"),
                    fill="tozeroy", fillcolor="rgba(74,111,165,0.15)",
                ))
                ...
                st.caption("Aún no hay ventas para mostrar. ¡Registra tu primera venta!")
        else:
            st.caption("Aún no hay ventas para mostrar. ¡Registra tu primera venta!")
    ...
    st.markdown("""<div class="win-section-title">
<h3>👕 Mis Productos</h3>
</div>""", unsafe_allow_html=True)

    busqueda_dash = st.text_input("Buscar producto...", placeholder="🔍 Buscar producto...", label_visibility="collapsed", key="buscar_dash")

    df_dash = df.copy()
    if busqueda_dash.strip():
        q = busqueda_dash.strip().lower()
        df_dash = df_dash[
            df_dash["ID"].astype(str).str.lower().str.contains(q)
            | df_dash["Producto"].astype(str).str.lower().str.contains(q)
        ]
    df_dash = df_dash.head(4)

    if not df_dash.empty:
        cols_dash = st.columns(4)
        for idx, (_, row) in enumerate(df_dash.iterrows()):
            with cols_dash[idx]:
...
<div class="win-prod-precio">{moneda(precio_normal)}</div>
</div>
</div>""", unsafe_allow_html=True)
                ...
                with c1:
                    if st.button("⭐", key=f"dash_fav_{row['ID']}", use_container_width=True, help="Favorito"):
                        ...
                        if actualizar_prenda(row["ID"], datos_act):
                            st.rerun()
                with c2:
                    if st.button("🎁", key=f"dash_of_{row['ID']}", use_container_width=True, help="Oferta"):
                        st.session_state.menu_activo = "existencias"
                        st.rerun()
                with c3:
...
<span style="font-size: 11px; color: var(--text-secondary);">Alerta mín: {row['alerta']}</span>
    ...
    # -----------------------------------------------------------------------------
        ...
        st.markdown(
            """
<div class="page-header">
    <div class="page-title">🏷️ Etiquetas y Fichas de Producto</div>
    <div class="page-subtitle">Genera fichas digitales para WhatsApp o etiquetas físicas para imprimir.</div>
</div>
""",
            unsafe_allow_html=True,
        )
        ...
        if df.empty:
            ...
            tab_ficha, tab_etiqueta = st.tabs(["📱 Ficha Digital (WhatsApp)", "🏷️ Etiquetas Físicas (Imprimir)"])
...
🛒 Ver catálogo: {URL_CATALOGO_WEB}"""
            ...
            with tab_etiqueta:
                st.markdown("<div class='section-subtitle'>Genera un PDF con etiquetas para imprimir (3 columnas x 8 filas por hoja A4).</div>", unsafe_allow_html=True)
...
{bs_txt}
                    ...
                    st.markdown("<div class='section-title'>📥 Descargar o Imprimir</div>", unsafe_allow_html=True)

                    lista_prendas_seleccionadas = []
                    for pid in prendas_sel_et:
                        fila_sel = df[df["ID"].astype(str) == str(pid)].iloc[0]
                        lista_prendas_seleccionadas.append(fila_sel.to_dict())

                    pdf_etiquetas = generar_etiquetas_pdf(lista_prendas_seleccionadas, tasa_etiqueta)

                    col_btn_pdf, col_btn_print = st.columns(2)
                    with col_btn_pdf:
                        if pdf_etiquetas:
                            st.download_button(
                                "📥 Descargar Etiquetas (PDF)",
                                data=pdf_etiquetas,
                                file_name="etiquetas_lewin.pdf",
                                mime="application/pdf",
                                use_container_width=True,
                                key="btn_descargar_etiquetas",
                            )
                        else:
                            st.caption("⚠️ No se pudo generar el PDF.")
...
<a class="link-imprimir" href="data:application/pdf;base64,{pdf_b64}" target="_blank">
    🖨️ Abrir e Imprimir
                    ...
                    st.caption(f"📄 {len(lista_prendas_seleccionadas)} etiqueta(s) — 3 columnas x 8 filas por página A4")
