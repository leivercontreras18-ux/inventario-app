...
# --- ESTILOS NÍTIDOS UI (TEMA DORADO / CHAMPAGNE Y NEGRO/AZUL OSCURO) ---
st.markdown(
    """
<style>
...
/* Fondo General: Medianoche con sutiles destellos dorados */
.stApp { 
    background: radial-gradient(circle at 20% 20%, rgba(245, 158, 11, 0.06) 0%, transparent 40%),
...
/* Efecto 3D Puerta para TODOS los botones (Tono Dorado / Champagne) */
...
div.stButton > button:hover, div[data-testid="stFormSubmitButton"] > button:hover {
    border-color: #fbbf24 !important;
    box-shadow: inset 20px 0 30px rgba(251, 191, 36, 0.15), 0 0 20px rgba(251, 191, 36, 0.4) !important;
    color: #ffffff !important;
}
...
div.stButton > button:active, div[data-testid="stFormSubmitButton"] > button:active {
    ...
    box-shadow: 0 0 5px rgba(251, 191, 36, 0.6) !important;
}
...
.metric-value { font-size: 32px; font-weight: 800; color: #fbbf24 !important; margin-top: 8px; }
...
.user-profile { 
    ...
    border: 1px solid rgba(245, 158, 11, 0.3); margin-bottom: 20px;
    display: flex; align-items: center; gap: 12px;
}
.user-avatar {
    width: 36px; height: 36px; background: #fbbf24; color: #0d121e;
    font-weight: 800; border-radius: 50%; display: flex; align-items: center;
    justify-content: center; font-size: 15px; box-shadow: 0 0 15px rgba(251, 191, 36, 0.4);
}
.user-info-title { font-size: 9px; color: #fbbf24; text-transform: uppercase; letter-spacing: 1.5px; font-weight: 700; }
...
if not st.session_state.autenticado and st.session_state.etapa == "bienvenida":
    ...
    with col_centro:
        st.markdown(
            ...
            <style>
            @keyframes levitar {
                0% { transform: translateY(0px); filter: drop-shadow(0 10px 15px rgba(251,191,36,0.4)); }
                50% { transform: translateY(-12px); filter: drop-shadow(0 20px 25px rgba(251,191,36,0.7)); }
                100% { transform: translateY(0px); filter: drop-shadow(0 10px 15px rgba(251,191,36,0.4)); }
            }
            .hero-card::before {
                content: '';
                position: absolute;
                ...
                background: radial-gradient(circle at 50% 50%, rgba(245, 158, 11, 0.08) 0%, transparent 55%);
                pointer-events: none;
                z-index: 0;
            }
            .hero-content {
...
<div class="metric-card">
    ...
    <div class="metric-value" style="color: {'#fbbf24' if total_alertas > 0 else '#fbbf24'} !important;">{total_alertas}</div>
</div>
""",
                unsafe_allow_html=True,
            )
...
""",
            ...
            if total_registros > 0:
                ...
                for idx, (_, row) in enumerate(df_paginado.iterrows()):
                    ...
                    borde_color = "#fbbf24" if is_alerta else "rgba(245, 158, 11, 0.25)"
                    badge_stock = f"<span style='color: #fbbf24; font-weight: 700;'>Stock Bajo ({row['cantidad']})</span>" if is_alerta else f"<span style='color: #22c55e; font-weight: 700;'>Stock: {row['cantidad']}</span>"
                        ...
                        st.markdown(
                            ...
                            ">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                    <span style="background: rgba(245, 158, 11, 0.15); color: #fbbf24; padding: 3px 8px; border-radius: 6px; font-size: 11px; font-weight: 700;">ID: {row['ID']}</span>
