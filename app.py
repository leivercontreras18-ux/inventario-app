...
st.set_page_config(page_title="Essence // Inventario Boutique", page_icon="👕", layout="wide")
...
def cargar_datos():
    ...
    creds = Credentials.from_service_account_file("inventario-app-506520-3b8ed4d70fe5.json", scopes=scopes)
...
try:
    ...
    menu = st.sidebar.selectbox("Navegación del Sistema", ["📊 Estado de Existencias", "➕ Registrar Prenda"])
    ...
    if menu == "📊 Estado de Existencias":
        ...
        if not df.empty:
            ...
            if "Cantidad" in df.columns and "Minimo" in df.columns:
                ...
                if not stock_bajo.empty:
                    st.warning(f"⚠️ Atención: Hay {len(stock_bajo)} modelo(s) con stock por debajo del mínimo recomendado.")
        ...
        with st.form("form_ropa"):
            ...
            categoria = st.selectbox("Categoría", ["Vestidos", "Blusas", "Pantalones", "Jeans", "Chaquetas", "Calzado", "Accesorios"])
            ...
            if st.form_submit_button("Guardar Prenda en el Sistema"):
                sheet.append_row([sku, nombre, categoria, talla, color, cantidad, minimo])
